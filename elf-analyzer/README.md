...
An interactive command-line tool for inspecting and manipulating **32-bit ELF** object files — similar in spirit to `readelf`, written from scratch in C.
...
- Toggle debug output. Files are opened with `mmap` for zero-copy access, and up to two files can be loaded at once.
...
## Concepts
ELF32 layout (`Elf32_Ehdr`, `Elf32_Shdr`, `Elf32_Sym`), section & string tables, `mmap`-based file access, and a function-pointer menu dispatch table.
