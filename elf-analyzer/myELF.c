...
void examine_elf_file()
{
    ...
    Elf32_Shdr *shdrs = (Elf32_Shdr *)((char *)map_start + header->e_shoff);
    if ((size_t)shdrs[header->e_shstrndx].sh_offset >= (size_t)file_size)
    {
        printf("Invalid section string table offset.\n");
        munmap(map_start, file_size);
        close(fd);
        return;
    }
...
void check_files_for_merge()
{
    ...
    for (int i = 0; i < file1->header->e_shnum; i++)
    {
        if (file1->section_headers[i].sh_type == SHT_SYMTAB)
        {
            ...
            if (symtab1->sh_link < file1->header->e_shnum)
    ...
    for (int i = 0; i < file2->header->e_shnum; i++)
    {
        if (file2->section_headers[i].sh_type == SHT_SYMTAB)
        {
            ...
            if (symtab2->sh_link < file2->header->e_shnum)
...
void merge_elf_files()
{
    ...
    for (int i = 0; i < file1->header->e_shnum; i++)
    {
            ...
            strcmp(section_name, ".rodata") == 0)
        {
            ...
            if (!section2){
                if (strcmp(section_name, ".text") == 0 || strcmp(section_name, ".data") == 0 || strcmp(section_name, ".rodata") == 0) {
                    printf("Warning: Mergeable section '%s' not found in second file.\n", section_name);
                }
                continue;
            }
        ...
        else
        {
            ...
            // SHT_NOBITS (e.g. .bss) occupies no file space — don't copy data.
    ...
    // === Bonus: Resolve undefined symbols from file1 using file2 ===
Elf32_Shdr *symtab1 = NULL, *symtab2 = NULL;
Elf32_Shdr *strtab1 = NULL, *strtab2 = NULL;

// Locate .symtab and .strtab in both files
for (int i = 0; i < file1->header->e_shnum; i++) {
    if (file1->section_headers[i].sh_type == SHT_SYMTAB) {
        symtab1 = &file1->section_headers[i];
        if (symtab1->sh_link < file1->header->e_shnum)
...
if (symtab1 && symtab2 && strtab1 && strtab2) {
    Elf32_Sym *symbols1_orig = (Elf32_Sym *)((char *)file1->map_start + symtab1->sh_offset);
    Elf32_Sym *symbols1 = malloc(symtab1->sh_size);
    if (!symbols1) { perror("malloc"); free(merged_section_headers); close(out_fd); return; }
    memcpy(symbols1, symbols1_orig, symtab1->sh_size);
    Elf32_Sym *symbols2 = (Elf32_Sym *)((char *)file2->map_start + symtab2->sh_offset);
    char *strs1 = (char *)file1->map_start + strtab1->sh_offset;
    char *strs2 = (char *)file2->map_start + strtab2->sh_offset;
    int num_syms1 = symtab1->sh_size / sizeof(Elf32_Sym);
    int num_syms2 = symtab2->sh_size / sizeof(Elf32_Sym);

    for (int i = 1; i < num_syms1; i++) {
        if (symbols1[i].st_shndx == SHN_UNDEF) {
            char *name1 = strs1 + symbols1[i].st_name;

            for (int j = 1; j < num_syms2; j++) {
                char *name2 = strs2 + symbols2[j].st_name;

                if (strcmp(name1, name2) == 0 && symbols2[j].st_shndx != SHN_UNDEF) {
                    symbols1[i].st_value = symbols2[j].st_value;
                    symbols1[i].st_shndx = symbols2[j].st_shndx;
                    if (debug_mode) {
    ...
    // Overwrite the .symtab section in out.ro with resolved symbols
    int symtab_index = symtab1 - file1->section_headers;
    Elf32_Shdr *merged_symtab = &merged_section_headers[symtab_index];
    lseek(out_fd, merged_symtab->sh_offset, SEEK_SET);
    write(out_fd, symbols1, symtab1->sh_size);
    free(symbols1);
}
...
int main()
{
    ...
    while (1)
    {
        ...
        if (scanf("%d", &choice) != 1)
        {
            ...
            // Non-numeric input: discard the rest of the line and retry
            int ch;
            while ((ch = getchar()) != '\n' && ch != EOF);
